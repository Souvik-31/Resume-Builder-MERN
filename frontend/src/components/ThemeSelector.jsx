...
const ThemeSelector = ({ selectedTheme, setSelectedTheme, resumeData, onClose }) => {
    ...
    const initialIndex = resumeTemplates.findIndex((item) => item.id === selectedTheme);
    ...
    return (
        <div className='max-w-7xl mx-auto px-4'>
            <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 p-4 sm:p-6 bg-white
        bg-gradient-to-r from-white to-violet-50 border border-violet-100 rounded-2xl shadow-sm'>
                <Tabs tabs={TAB_DATA} activeTab={tabValue} setActiveTab={setTabValue} />
                <button className='w-full sm:w-auto flex justify-center items-center gap-3 py-3 px-6
            bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-black rounded-2xl
            hover:scale-105 transition-all shadow-lg hover:shadow-xl' onClick={handleThemeSelection}>
                ...
                <div className='lg:col-span-2 bg-white rounded-xl border border-gray-100 p-4 sm:p-6'>
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[60vh] lg:max-h-[70vh] overflow-auto p-2'>
                        {resumeTemplates.map((template, index) => (
                            <TemplateCard
                                key={`template_${template.id || index}`}
